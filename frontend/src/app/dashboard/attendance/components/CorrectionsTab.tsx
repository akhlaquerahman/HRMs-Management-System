...
export function CorrectionsTab() {
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-gray-500 animate-pulse">Loading corrections queue...</div>;
  }
      ...
      <Table>
        ...
        <TableBody>
          {corrections.length === 0 ? (
            <TableRow><TableCell colSpan={6} className="h-32 text-center text-gray-500">No pending attendance corrections.</TableCell></TableRow>
          ) : (
            corrections.map((req: any) => (
              <TableRow key={req.id}>
                <TableCell>
                  <div className="flex items-center gap-3">
                    <Avatar className="h-9 w-9"><AvatarFallback>{req.employee?.firstName?.[0]}{req.employee?.lastName?.[0]}</AvatarFallback></Avatar>
                    <div>
                      <p className="font-medium text-sm text-gray-900">{req.employee?.firstName} {req.employee?.lastName}</p>
                ...
                <TableCell className="text-right space-x-2">
                  <Button variant="outline" size="sm" className="text-red-600 border-red-200 hover:bg-red-50" onClick={() => handleAction(req.id, 'reject')}>
